...
def parse_setting_file(file_path):
    with open(file_path, 'r') as file:
        content = file.read()
    
    pattern = r'rank_(\d+)_model_(\d+)_ptm_seed_\d+ pTMscore:(\d+\.\d+)'
    match = re.search(pattern, content)
    rank_number = int(match.group(1))
    model_number = int(match.group(2))
    pTMscore = float(match.group(3))
    
    pattern = r'model_\d+_ptm_seed_\d+ recycles:\d+ tol:(\d+\.\d+) pLDDT:(\d+\.\d+) pTMscore:(\d+\.\d+)'
    ...
    for match in matches:
        if int(match.group(1)) == model_number:
            metrics['tol'] = float(match.group(2))
            metrics['pLDDT'] = float(match.group(3))
            break
            
    return metrics

def draw_heatmap(file_path, cmap='bwr', output_prefix=None, threshold=None):
    setting_file_path = file_path.replace(".raw.txt", ".setting.txt")
    metrics_from_setting = parse_setting_file(setting_file_path)
    try:
    ...
    return {
        ...
        "suffix_i": suffix_i,
        "suffix_j": suffix_j,
        "model_number": metrics_from_setting["model_number"],
        "tol": metrics_from_setting["tol"],
        "pLDDT": metrics_from_setting["pLDDT"],
    }
