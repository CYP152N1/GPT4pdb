...
if __name__ == "__main__":
    ...
    # Plot the metrics in a single line chart
    plt.figure(figsize=(10, 6))
    plt.plot(metrics_df["rank"], metrics_df["avg_pae_ij"], label="Average pae_ij")
    plt.plot(metrics_df["rank"], metrics_df["avg_pae_ji"], label="Average pae_ji")
    plt.plot(metrics_df["rank"], metrics_df["sum_p_cbcb_div_threshold"], label="Sum p(cbcb<8) / threshold")
    plt.plot(metrics_df["rank"], metrics_df["sum_p_cbcb_div_i_max_minus_threshold"], label="Sum p(cbcb<8) / (i_max - threshold)")
