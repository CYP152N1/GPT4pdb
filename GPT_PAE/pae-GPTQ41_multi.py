...
<body>
    ...
    <div class="image-container">
        {% for img_filename in img_filenames %}
            {% if '_i_sum_bar.png' in img_filename or '_j_sum_bar.png' in img_filename %}
                {% set rank_num = img_filename.split('_')[1] %}
                {% set file_num = img_filename.split('_')[4][:-4] %}
                {% set file_type = 'i' if '_i_sum_bar.png' in img_filename else 'j' %}
                {% set new_filename = 'rank_' + rank_num + '_' + file_type + '_sum_bar_' + file_num + '.png' %}
                <div class="image-wrapper">
                    <img src="{{ new_filename }}" alt="{{ new_filename }}">
                </div>
            {% endif %}
        {% endfor %}
    </div>
